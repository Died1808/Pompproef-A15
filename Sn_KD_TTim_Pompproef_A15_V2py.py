...
##############################################K-waarde        
    ...
    def df_TTIM(self, filename):
        ...
        model komen. De xY coordinaten zijn in RD_Amersfoort (crs:28992).
        Verder wordt op basis van een shapefile de net ten noorden van deze
        locatie liggende haven (Eem.shp) in het model opgenomen met een diepte tot -20
        m NAP. 
        De lagen waar de meetfilters in staan wordt tussen laag 135/140 in het 
        model verwacht, daar wordt dan ook de verlagingslijnen vanberekend.
        ...
        #pompput
        tf = -17 # top Filter
        bf = -23 # onderzijde filter
        tfind = df['depth'].sub(tf).abs().values.argmin()
        bfind = df['depth'].sub(bf).abs().values.argmin() 
        well1 = np.arange(tfind, bfind, 1)
        w1 = Well(ml, xw=86920, yw=432048, rw=0.25, tsandQ=[(0,Qm)], layers=well1)

        tH = -19 #laag waar Haven intreedt
        bH = -20 #Bodem haven
        tfind = df['depth'].sub(tH).abs().values.argmin()
        bfind = df['depth'].sub(bH).abs().values.argmin() 
        Eemlagen = np.arange(tfind, bfind, 1)
        shape = fiona.open("Eem.shp")
        first = next(iter(shape))
        coords= first['geometry']['coordinates']
        Haven = HeadLineSinkString(ml, xy=coords,tsandh=[(0,0)], res=1, wh=5, layers=Eemlagen)
        ...
        De daling op 23 meter afstand en vlak bij de bron worden als + in de figuur
        opgenomen. 
        ...
        """
        laag = np.arange(135,136,1)
        kleuren(len(laag))
        
        #Gemeten peilbuizen
        mat=pd.read_excel('Pb-PP.xlsx', index_col=None, skiprows=0, engine='openpyxl')  
        x=mat['tijd']/1440
        y1=mat['PB9514']
        y2=mat['bron']

        for la in laag:
            ml.xsection(x1=86920, x2=86920, y1=432048, y2=432048+100, npoints=1000, t=2, layers=[la], lw=3, newfig = False)
        ...
        KD = round((df['k'].sum()/5+onderlaag*df.iloc[-1,1]),0)        
        title = (filename +' PP_A15'+ '   KD = '+ str(int(KD))+ ' [m2/dag]')
        plt.title(title, fontsize=12, pad=10)
        plt.plot( 23,-1.25, 'k+', markersize=20)
        plt.plot(0.34,-4.68, 'k+', markersize=20)
        plt.savefig('Afstand.png', bbox_inches='tight')
        plt.show()
    
        for la in laag:
            t=np.logspace(-4,1,1000)
            s1=ml.head(86926,432029,t)
            s2=ml.head(86920.34,432048,t)
            plt.semilogx(t,s2[la]*-1, lw=3, alpha=0.5, linestyle='dotted')
            plt.semilogx(t,s1[la]*-1, lw=3, alpha=0.5)
        ...
        plt.grid(axis='both')
        plt.plot(x,y1, '+', color='black', markersize=3)
